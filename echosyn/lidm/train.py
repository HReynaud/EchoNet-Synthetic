...
def log_validation(
    ...
    indices = np.random.choice(len(val_dataset), size=config.validation_count, replace=False)
    ...
    with torch.no_grad(), torch.autocast("cuda"):
        ...
        B, C, H, W = config.validation_count, 4, config.unet.sample_size, config.unet.sample_size
        latents = torch.randn((B, C, H, W), device=accelerator.device, dtype=weight_dtype, generator=generator)
    ...
    with torch.no_grad(): # no autocast
        ...
        images = torch.cat([ref_frames, images], dim=2) # B x C x (2 H) x W // vertical concat
